...
class RouteGuideClient(host: String, port: Int) {
  ...
  def getFeature(lat: Int, lon: Int): Unit = {
    ...
    try {
      ...
      val lat = RouteGuideServiceUtil.getLatitude(feature.getLocation)
      val lon =  RouteGuideServiceUtil.getLongitude(feature.getLocation)
...
//  }
  ...
  val features = RouteGuideUtil.getFeatures
