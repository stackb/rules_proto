...
ProtoLibraryAspectNodeInfo = provider(
    fields = {
        ...
        "output_dirs": "The directories generated by this aspect and its transitive dependencies, as a dict indexed by the root directory",
    },
)
...
def proto_compile_impl(ctx):
    ...
    output_dirs = [dep[ProtoLibraryAspectNodeInfo].output_dirs for dep in ctx.attr.deps]
    ...
    if output_dirs:
        ...
        copied_output_directories.append(output_dir)

        # Copy directories to shared output directory in one action
        dirs = ([d for output_dirs_dict in output_dirs for dirs in output_dirs_dict.values() for d in dirs])
        ctx.actions.run_shell(
            ...
            progress_message = 'copying directories to {}'.format(output_dir.path),
        )
    ...
    return [
        ProtoCompileInfo(
            label = ctx.label,
            outputs = copied_output_files + copied_output_directories,
            files = copied_output_files + copied_output_directories,
        ),
        DefaultInfo(
            files = depset(copied_output_files + copied_output_directories),
            data_runfiles = ctx.runfiles(files=copied_output_files + copied_output_directories),
        )
    ]
...
def proto_compile_aspect_impl(target, ctx):
    ...
    outputs_files_dict = {full_outdir: output_files}
    outputs_dirs_dict = {full_outdir: output_dirs}

    for transitive_info in transitive_infos:
        for root, files in transitive_info.output_files.items():
            if root not in outputs_files_dict:
                outputs_files_dict[root] = []
            outputs_files_dict[root].extend(files)

        for root, dirs in transitive_info.output_dirs.items():
            if root not in outputs_dirs_dict:
                outputs_dirs_dict[root] = []
            outputs_dirs_dict[root].extend(dirs)

    return [
        ProtoLibraryAspectNodeInfo(
            output_files = outputs_files_dict,
            output_dirs = outputs_dirs_dict,
        )
    ]
