...
_nodejs_grpc_compile_aspect_rule = proto_compile_aspect_rule(_nodejs_grpc_compile_aspect)

_nodejs_grpc_compile_rule = proto_compile_rule(_default_plugins)

def nodejs_grpc_compile(**kwargs):
    if kwargs.pop("transitive", False):
        proto_compile_aspect_rule_macro(_nodejs_grpc_compile_aspect_rule, **kwargs)
    else:
        _nodejs_grpc_compile_rule(**kwargs)
