...
// (snake-case, so 'protoc:js:closure' -> protoc_js_closure.go).
//
// (2) if the plugin corresponds to a protoc "builtin" such as java, the name of
// the plugin is 'protoc:java' (reflects the value for the --java_out argument).
// Variants are appended to that name, such as 'protoc:js:common' (commonjs) and
// 'protoc:js:closure' (closure).
//
